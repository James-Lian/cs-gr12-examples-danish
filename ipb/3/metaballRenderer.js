...
// Raymarching
class MetaBallRenderer {
	constructor({ scene = [], posX, posY, dotSize = 1, spacingX, spacingY, resolutionX = 300, resolutionY = 300, camZ = 20, rotationSpd = 0.1, fieldStrength = 0.8	}) {
		this.scene = scene;
	...
	update() {
		...
		let camDim = Math.max(Math.floor(this._findCameraDimensions() / (1 / 1.2)), this.maxCamDim);
		this.maxCamDim = camDim;
			...
			for (let y = 0; y < numPixelsY; y++) {
				...
				if (this._closestBall) {
					...
					if (depth < maxDist) {
						...
						// creating an "average position" of the blob's rendered pixels
						if (averageBlobPos.hasOwnProperty(this._closestBall.text)) {
							averageBlobPos[this._closestBall.text]["x"] += this.pos.x + rayX;
							averageBlobPos[this._closestBall.text]["y"] += this.pos.y + rayY;
						} else {
							averageBlobPos[this._closestBall.text] = {"x": 0, "y": 0, "num": 0}
							averageBlobPos[this._closestBall.text]["x"] = this.pos.x + rayX;
							averageBlobPos[this._closestBall.text]["y"] = this.pos.y + rayY;
						}
						averageBlobPos[this._closestBall.text]["num"] += 1
						
						if (labelledBlobs.includes(this._closestBall) === false) {

							// calculating center
							// let labelX = map(map(this._closestBall.pos.x, -camDim, camDim, 0, numPixelsX), 0, numPixelsX, -this.resolutionX, this.resolutionX);
							// let labelY = map(map(this._closestBall.pos.y, -camDim, camDim, 0, numPixelsY), 0, numPixelsY, -this.resolutionY, this.resolutionY);
							// this._closestBall.showLabel(this.pos.x + labelX, this.pos.y + labelY);
		...
		for (let blob of labelledBlobs) {
			let labelX = averageBlobPos[blob.text]["x"] / averageBlobPos[blob.text]["num"];
			let labelY = averageBlobPos[blob.text]["y"] / averageBlobPos[blob.text]["num"];
			blob.showLabel(labelX, labelY)
		}
		this._rotateBalls();
	}
}
